...
def test_handler_cognito_token(mocker):
    mock_verify_cognito_token = mocker.patch.object(verify_cognito_token, "verify_cognito_token")
    ...
    request = json.load((Path(__file__).parent / "data/authorization_request.json").open("rt"))
    response = main.handler(request, {})
    expected = AuthorizationResponse(
...
def test_handler_general_exception(mocker):
    mocker.patch.object(util, "verify_dev_token", side_effect=Exception("Unexpected Error"))
    request = json.load((Path(__file__).parent / "data/authorization_request.json").open("rt"))
    response = main.handler(request, {})
    assert response == {"message": "An unexpected error occurred: Unexpected Error", "statusCode": 500}

def test_handler_unsupported_authorization_type(mocker):
    request = {"type": 'FOO', 'authorizationToken': 'bar', 'methodArn': 'some-arn'}
    response = main.handler(request, {})
    assert 'Validation error: 1 validation error for TokenAuthorizationRequest' in response['message']
...
class TestGetOwnerUuid(unittest.TestCase):
    ...
    def test_get_owner_uuid_success(self, mock_boto3_client):
        ...
        owner_response = OwnerResponse(owner=Owner(owner_uuid=owner_uuid, first_name="John", last_name="Doe", email_address="john.doe@example.com", phone_number="1234567890"))

        # Creating a MagicMock for the lambda client
        mock_lambda_client = MagicMock()
        mock_lambda_client.invoke.return_value = {'Payload': io.BytesIO(owner_response.json().encode('utf-8'))}
